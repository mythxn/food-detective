...
def root():
    ...
    try:
        cursor.execute(
            "insert into recipeOfTheDay VALUES (:name, :desc, :link, :img)", params)
    except:
...
def gb():
    ...
    page = requests.get(
        "https://weather.com/en-IN/weather/tenday/l/af60f113ba123ce93774fed531be2e1e51a1666be5d6012f129cfa27bae1ee6c")
    ...
    for items in table:
        for i in range(len(items.find_all("tr"))-1):
            d = {}
            try:
                d["date"] = items.find_all(
                    "span", {"class": "day-detail"})[i].text
                d["desc"] = items.find_all(
                    "td", {"class": "description"})[i].text
                d["temp"] = items.find_all("td", {"class": "temp"})[i].text
                d["wind"] = items.find_all("td", {"class": "wind"})[i].text
                d["humidity"] = items.find_all(
                    "td", {"class": "humidity"})[i].text

                # save into db
                connection = sqlite3.connect(DB_FILE)
                cursor = connection.cursor()
                params = {'date': d["date"], 'desc': d["desc"],
                          'temp': d["temp"], 'wind': d["wind"], 'humidity': d["humidity"]}
                try:
                    cursor.execute(
                        "insert into weather VALUES (:date, :desc, :temp, :wind, :humidity)", params)
                except:
                    print("Already in DB; Not updating.")
                connection.commit()
                cursor.close()
...
def _newaccount(username, email, password):
    ...
    cursor = connection.cursor()
    cursor.execute(
        "insert into accounts VALUES (:username, :email, :password)", params)
    connection.commit()
    cursor.close()
...
def register():
    _newaccount(request.form['username'],
